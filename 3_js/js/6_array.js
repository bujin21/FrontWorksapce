test1.onclick = function(){
    ...
    /*
        2. for .. of문
         - java의 향상된 반복문과 가장 비슷한 구문으로 배열유형의 데이터를 반복시키기 위해 사용된다.
...
test2.onclick = function(){
    ...
    var arr3 = [
        1,2,3,
        "안녕", true, {id:"mkm"}, {'a':'b'}, function(){console.log('zz')}
    ]
    arr3[7]();
};
...
test3.onclick = function(){
    ...
    console.log(arr.lastIndexOf('melone'));//-1
    console.log(arr.indexOf('strawbery'));//-1
};
...
/* 
    push : 배열 마지막에 요소를 추가한 후, 변경된 길이를 변환
...
test14.onclick = function(){
    var objArr = [
        {id : 'wjsqnwls', name : '전부진'},
        {id : 'wjsqnwls2', name : '전부진2'},
        {id : 'wjsqnwls3', name : '전부진3'},
        {id : 'wjsqnwls4', name : '전부진4'},
    ];
    ...
    // 존재하지 않을 경우 undefined반환
    console.log(findOne);

    var filteredArray = objArr.filter(function(obj){
        if(obj.id.includes('wjsqnwls')){
            return true;
        }
    });
...
test15.onclick = function(){
    //1.
    var drinks = [];
    var drink = document.querySelectorAll("#drink > li").forEach(function(li){
        drinks.push(li.innerHTML);
    });
    console.log(drinks);
    //2.
    var findOne = drinks.find(function(li){
        return li.includes('ff');
    });
    console.log(findOne);

    // 3.
    var filteredArr = drinks.filter(function(li){
        return li.includes('o');
    });
    console.log(filteredArr);

    // 4
    var priceArr = [2000, 1600, 4000, 8000, 15000];
    var mappedDrink = drinks.map(function(d, i){
