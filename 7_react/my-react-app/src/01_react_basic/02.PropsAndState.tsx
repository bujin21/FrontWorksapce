...
/* 
    #1. Props
     ...
     - 단반향 데이터 흐름을 통해 컴포넌트의 상태를 예측하기 쉽게 만든다.
   ...
   let [message, setMessage] = useState('Hello From Parent');
    ...
    /* 
        #2. State
         ...
         - 부모컴포넌트에서 여러 자식컴포넌트에 공유해야할 데이터가 있는 경우 
         부모컴포넌에서 state를 선언후 자식에게 props로 전달해야 한다.
    */
...
function ChildComponent({style, message, setMessage}:ChildProps){
    //console.log(props);
    // props.message = "마음에 안드네..."; // props는 읽기전용으로 사용할것
    ...
    return (
        <div>
            ...
            <button onClick={messageHandeler}>messageHandeler</button>
