...
function OptimizationPractice() {
  ...
  // 검색버튼 클릭 시에만 연산 실행되도록 별도 state 추가
  
  // inputRef로 input에 접근, 버튼 클릭 시에만 searchKeyword 업데이트
  const onSearch = () => {
  ...
  // useMemo로 메모이제이션 + 의존성을 searchKeyword로 설정
  //       → 검색버튼 클릭(searchKeyword 변경) 시에만 heavyFilter 실행
  const filtered = useMemo(() => heavyFilter(users, keyword),
    [users, keyword]
  );

  // useCallback으로 showCount 메모이제이션
  //       → filtered가 바뀌지 않으면 같은 함수 참조 유지
