/* 
    1. class
     - javascript의 class문법을 확장하여 
     좀더 객체지향적인 프로그래밍이 가능하도록 만들었다.
...
const tiger = new Tiger('jbj', 25, '서울');
tiger.bark();
...
const result2 = returnGeneric("hello"); // 자동타입추론이 된다.
...
function config<T extends Options = {theme:'dark'}>(options?:T):T{ //= {theme:'dark'}는 값이 없을 때 기본값
...
/* 
    4. Utility Type
     ...
     1) Partial<T>
      ...
      - 모든 속성을 필수, 읽기 전용으로 만들어 주는 Required<T>,
       Readonly<T>도 존재한다.
*/
interface User {
    id:number;
...
const userData:NewUser = {name:"jbj", email:"zz"};
/*
    Utility Type 조합
    1. User에서 id를 제외한 나머지 속성을 선택한 후 전체 선택속성으로 만들기.
...
*/
function getUser(){
    return {id :1, name:"jbj", email:'aaa'};
}
