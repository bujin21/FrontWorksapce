...
/* 
    3. Optional Paramter
     ...
     - 변수명? : type
     - 옵셔녈파라미터는 type | undefined 유니언 타입으로 설정
*/
function fnOptional(a:number, b?:number) : number {
    // 디펜시브 코딩 : null이 오는걸 예상 혹은 방지하기 위한 코드
    if( b != undefined){
        return a * b;
    }
    return 0;
...
function objectDesFn2({
    ...
    // b : string,
    // c : boolean
    [key:string] : string | boolean | number // 공통체크
}){

}
objectDesFn2({a:111, b:'jbj', c:false});
...
// not null 단언문
// - 선택한 변수의 값이 null(undefind포함)이 아님을 단언하는 문법
...
/*
    9. never type
     - 어떤 값도 가질수 없는 타입을 의미하며, 절대 값을 반환할 수 없는 함수나,
