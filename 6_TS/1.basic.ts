/* 
    1. 타입주석
     ...
     - 타입주석을 통해 정적 타입 검사가 가능해지며, 
       잘못된 타입의 값이 들어갈 셩우 컴파일 에러를 발생시킨다.
*/
...
/* 
    2. 타입추론
     ...
     - 간단한 변수 선언시 타입주석을 작성하지 않고 타입추론을 활용하는 것이 좋다.
     - 단, 복잡한 함수의 매개변수나 반환형, api명세등 명확한 타입정의가 필요한 곳에는 반드시 작성한다.
*/
...
/* 
    튜플
     ...
     - 일반배열은 언제든 요소의 수가 변할 수 있지만, 튜플은 항상
      고정된 구조를 가짐.
*/
let numTuple : [string, number, number];
numTuple = ["a",2,3];
//console.log(numTuple[3]); // 잘못된 인덱스 접근시 컴파일에러
console.log(numTuple[2]); // 잘못된 인덱스 접근시 컴파일에러
...
/*
    4. readonly
     ...
     - 타입스크립트의 정적타입체크 시스템은 실제 코드를 실행하기 전 단계에서 작동하므로
       실행환경에서 동작하는 메서드에 대한 검사는 수해하지 않는다.
*/
numTuple.push(1111);
...
readonlyObj = {name : 'kh'};
readonlyObj.name = 'jbj';

let readonlyObj2 : {readonly name : string};
readonlyObj2 = {name : 'kh'};
//readonlyObj2.name = 'jbj';

/* 
    5. any Type
     - 어떤 값이든 저장할 수 있는 타입
     - 타입스크립트의 정적타입검사를 우회하기 위해 설계되었다.
     - javascript코드와의 호환성 유지 및 마이그레이션을 돕기위한 타입이고,
