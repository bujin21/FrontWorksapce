...
const person1:Person = {name:'jbj', age:24};
const person2:Person = {name:'jbj', age:24, gender:'male'};
...
export type Reponse = SuccessResponse | FailResponse;
type str = string;
type bool = boolean;

/*
    5. 인덱스 시그니쳐
     - 객체의 속성명에 대하여 명확히 인지 못하는 경우
     사용하는 문법
     - 객체의 속성몇과 그 타입을 "일반환" 하여 표현한다.
...
/*
    6. 타입호환성
     - 타입스크립트에서 두 타입이 서로 대입 가능한지를 판단 하는 개념
...
const jbj:Teacher = {name:'jbj', age:32};
const jbj2:Student = jbj; // 구조 똑같기 때문에 호환가능

// 객체 리터럴 대입시, 타입검사 실행.
//     - 잉여속성 검사
//const kim:Student = {name:'kim', age:5, location:'seoul'}
const kim = {name:'kim', age:5, location:'seoul'}
