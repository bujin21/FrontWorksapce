        ...
        .then( response => {
            ...
            /* #1. JWT
                 ...
                 - JWT를 활용한 인증/인가 매커니즘은 세션기반 로그인방식과 달리 서버가 인증상태를 따로
                  저장할 필요가 없기 때문에 무상태서버 구형이 가능하다.
                 - 발급된 JWT토큰은 매 요청시 함계 전달되어 인증에 사용한다.
               
               #2 JWT저장 위치
                - JWT 토큰은 클라이언트의 브라우저에서 관리해야하는 데이터이며, API요청에 사용되는 중요한 데이터이다.

                1) localStorage
                 - 브라우저의 로컬저장소 브라우저가 종료되어도 데이터가 유지된다.
                 - xss공격에 취약하다. 따라서 jwt와 같은 중요한 토큰은 보관하지 않는다.
                
                2) sessionStorage
                 - 탭단위 세션 저장소로 탭을 닫으면 데이터도 삭제된다. (데이터 유지시간이 짧다.)
                 - xss공격에 취약하다.
                
                3) 쿠키
                 - 쿠키는 특정 도메인의 경로에 지정된 시간동안만 저장된느 데이터
                 - 쿠키는 HTTP통신시 항상 자동으로 전달된다.
                 - 기본쿠키는 JS로 접근하여 탈취될 수 있으나 http-only설정시 js로 쿠키제어가 불가능.
                   (xss에서 안전)

                4) 리엑트메모리
                 - 리덕스 보관시 메모리에만 저장되며, 새로고침시 데이터가 소멸한다.
                ...
                #3. AccessToken과 RefreshToken
                 ...
                 - RefreshToken accessToken이 만료된경우 이를 갱신하기 위한 토큰.
